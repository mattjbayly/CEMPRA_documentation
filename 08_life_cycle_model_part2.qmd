...
-   **Spawner caps (`bh_spawners` / `hs_spawners`)**: Constrain the **total number of spawners across all mature stages** (counted as the maturity-weighted abundance, N × mat). For `hs_spawners`, this is a hard cap with proportional reduction across spawning age classes. For `bh_spawners`, K is redistributed proportionally across spawning stages and individual BH constraints are applied per-stage. In both model types, fish removed by the cap **die** — they are removed from the population vector, not held back from spawning. For anadromous (semelparous) populations this distinction is moot: excess returners die without spawning, which is what would happen anyway. For **non-anadromous (iteroparous) populations, the spawner capacity should therefore be interpreted as an *adult capacity***: excess adults are culled each year to hold the spawner count at K, so the cap limits the standing stock of the mature stages (and their entire future reproductive contribution), not just the current year's spawning. Individuals do not survive as non-breeders to spawn in a later year.
...
![Simulation of Steelhead Spawners with stage_1 capacity set to 160,000](images/image047.png){#fig-steelhead-spawners}
...
| `bh_spawners` | Beverton-Holt | Total spawner capacity across all mature age classes. Mature stages are identified via the maturity vector (mat \> 0) and spawners are counted as the maturity-weighted abundance (N × mat). When total spawners exceed the cap, K is redistributed proportionally across spawning stages, and BH is applied per-stage. Requires `k_stage_B_mean` in the habitat capacities file. |
| `hs_spawners` | Hockey-Stick | Total spawner hard cap across all mature age classes (counted as the maturity-weighted abundance, N × mat). When total spawners exceed the cap, all spawning stages are reduced proportionally. Requires `k_stage_B_mean` in the habitat capacities file. |

::: callout-note
**Interpretation for non-anadromous (iteroparous) populations:** fish removed by a spawner cap die — they are removed from the population, not held back from breeding — so `bh_spawners`/`hs_spawners` should be interpreted as an **adult capacity** in the non-anadromous model. Excess adults are culled each year to hold the (maturity-weighted) spawner count at K, limiting the standing stock of the mature stages and all of their future spawning, not just the current year's reproduction. In stages with partial maturity (0 \< mat \< 1), the proportional reduction also removes the co-occurring immature fraction of those stages.
:::
...
#### Optional: Capacity Uncertainty (`_cv` columns)

**This feature is entirely optional.** If you only supply `k_..._mean` columns (as in all the examples above), every capacity value is treated as exact and nothing described in this section applies — this is the default and recommended starting point.

If you want to represent **uncertainty in a capacity estimate**, add a companion `_cv` column holding the coefficient of variation for the matching `_mean` column. The `_cv` column name must mirror the `_mean` column name, for example:

| **Mean column**     | **Companion uncertainty column** |
|---------------------|----------------------------------|
| `k_stage_1_mean`    | `k_stage_1_cv`                   |
| `k_stage_Pb_1_mean` | `k_stage_Pb_1_cv`                |
| `k_stage_B_4_mean`  | `k_stage_B_4_cv`                 |
| `k_spawners_mean`   | `k_spawners_cv`                  |

When a capacity has a cv greater than zero, the model no longer uses a single fixed K for that stage. Instead, a new K value is drawn **for every simulation year and for every batch replicate** from a normal distribution with a mean equal to the `_mean` value and a standard deviation of `mean × cv`, truncated at zero (a draw can never produce a negative capacity). Within each batch replicate, the baseline run and the stressor (CE) run share the same sequence of capacity draws, so comparisons between the two are not confounded by different random capacities. If a cumulative-effects stressor also modifies that capacity, the two effects combine multiplicatively. The initial (year-0) population vector is always seeded from the mean capacities.

A few practical notes:

-   **Blank cells, NA values, or a missing `_cv` column all mean cv = 0** — the capacity is treated as exact and no random draws are made. You can mix exact and uncertain capacities freely within one file (e.g., an exact fry capacity alongside an uncertain spawner capacity).
-   Choose cv values thoughtfully: a cv of 0.1–0.3 represents modest uncertainty; values much above \~0.5 produce highly volatile year-to-year capacities (and increasingly frequent truncation at zero, which shifts the effective mean upward slightly).
-   **R package only:** capacity uncertainty is currently supported when running the model through the CEMPRA R package (`PopulationModel_Run()`). The Shiny web app does not use `_cv` columns — if a habitat capacities file containing them is uploaded to the app, the `_cv` columns are dropped and all K values are treated as exact (a note in the app's upload panel says the same).
...
![Compensation Ratio CR for life stage i](images/image048.png){#fig-compensation-ratio}
...
![Influence of compensation ratios on stage-specific survivorship](images/image049.jpg){#fig-compensation-ratios-effect}
...
Stable-stage distribution (portions) under the example species profile (@fig-stable-stage-distributions):

#### Stable Stage Distributions (as a fraction 0 - 1) & Stage-specific Carrying Capacities (K):

![Stable Stage Distributions and Stage-specific Carrying Capacities](images/image050.png){#fig-stable-stage-distributions}
...
Alternatively, the R package supports a comma-separated list of life stages in the `Life_stages` column to link a single stressor-response relationship to multiple vital rates (e.g., `stage_1, stage_2`). Internally, the code duplicates the stressor-response relationship for each life stage specified. See the [Life_stages Column](#the-stressor-response-workbook-life_stages-column) section below for more details on how to specify life stages. However, this comma-separated functionality is only supported in the R package and is not available in the R Shiny application. For simplicity and reproducibility, if a given stressor is linked to two or more vital rates, we recommend creating separate, distinctly named stressors in the Stressor-Magnitude and Stressor-Response workbooks rather than using a comma separator (e.g., `SummerStreamTemp_Parr`, `SummerStreamTemp_Prespawn`).

Note that each stressor name may appear only **once** on the Main worksheet: one stressor is always linked to exactly one stressor-response relationship. Repeating a stressor name on a second row (e.g., to target a second parameter such as capacity) is not supported, and `StressorResponseWorkbook()` will stop with an error identifying the duplicated name. Use a comma-separated `Life_stages` list to reach multiple life stages, or create distinctly named stressors (each with its own stressor-response worksheet) to affect multiple parameters. Unrecognized `Life_stages` or `Parameters` values do not stop a model run, but they have no effect and the population model will issue a warning so typos can be caught.
...
### The Stressor-Response Workbook 'Life_stages' Column {#the-stressor-response-workbook-life_stages-column}
...
| Tag                   | Target                             |
|-----------------------|------------------------------------|
| `eps`                 | Eggs per spawner (all stages/ages) |
| `eps_3`, `eps_4`, ... | Stage/age-specific fecundity       |
...
## Setting Up the Stressor Magnitude File

The stressor magnitude file defines the dose (mean, standard deviation, distribution, and limits) of each stressor at each location (HUC). For the full description of this file and its required columns, refer back to the [Stressor Magnitude File](https://mattjbayly.github.io/CEMPRA_documentation/05_data_inputs.html#stressor-magnitude-file) section of the Data Inputs chapter. The notes below describe an optional feature that is specific to the population model.

### Optional Stressor Activation Window (`From_Year` and `To_Year`)

By default, every stressor is applied to all years of a population model simulation. You can optionally restrict a stressor to a specific time window by adding two columns to the stressor magnitude file: `From_Year` and `To_Year`. When supplied for a given stressor–location combination, the stressor only affects vital rates between those simulation years (inclusive); outside that window the stressor is treated as having no effect. For example, if a stressor called `Drawdown` at location #44 is given a `From_Year` of 50 and a `To_Year` of 70, then `Drawdown` is switched off before simulation year 50 and after year 70, and is only active in years 50 through 70. This is useful for representing time-limited pressures such as a temporary construction impact, a planned restoration action that takes effect partway through a run, or a stressor that is only expected to occur during a defined period.

The feature is fully backwards compatible and entirely optional. If the `From_Year` and `To_Year` columns are absent, or if they are left blank for a particular stressor, that stressor is applied to all years exactly as before — existing stressor magnitude files do not need to change. The columns can be populated for only a subset of stressors; any stressor with missing values is simply applied across the whole simulation. You can also supply only one of the two bounds: providing only `From_Year` activates the stressor from that year onward (to the end of the run), while providing only `To_Year` runs the stressor from the start of the simulation up to that year. If `From_Year` is greater than `To_Year` the window is empty, so the stressor is treated as inactive for all years and a warning is issued to flag the likely data-entry error.

**One window per stressor.** Each stressor at a location supports a *single* activation window: supplying multiple rows for the same stressor–location combination with different `From_Year`/`To_Year` values stops the run with an error (the rows cannot be processed independently through the dose-sampling machinery). To model a stressor that occurs in **two or more separate periods** — for example a construction impact in years 1–5 and a drawdown in years 20–25 — give each period its **own stressor name** (e.g., `Drawdown_early` and `Drawdown_late`), each with its own row, window, and magnitude in the stressor magnitude file, and add a matching entry (which can share the identical response curve) for each name in the stressor-response workbook. Each named stressor is then processed fully independently, giving genuine multi-period behaviour.

Note that `From_Year` and `To_Year` are used **only by the population model**. They are ignored by the Joe Model, which continues to evaluate cumulative system capacity across all stressors without any time restriction. The years refer to the simulation time step (1 to the number of years in the run), not calendar years.
...
![Sample life cycle diagram and periodicity table for Coho Salmon](images/image055.png){#fig-coho-salmon-lifecycle}
